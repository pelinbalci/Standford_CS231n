...
solvers = {}
update_rule = 'adam'
for lr in [1e-4, 5e-4, 5e-3, 1e-3, 1e-2]:
  print('running with ', lr)
...
for lr, solver in list(solvers.items()):
  plt.subplot(3, 1, 1)
  plt.plot(solver.loss_history, 'o', label=lr)

  plt.subplot(3, 1, 2)
  plt.plot(solver.train_acc_history, '-o', label=lr)

  plt.subplot(3, 1, 3)
  plt.plot(solver.val_acc_history, '-o', label=lr)
